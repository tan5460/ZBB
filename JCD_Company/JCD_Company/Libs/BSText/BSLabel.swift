...
open class BSLabel: UIView, TextDebugTarget, TextAsyncLayerDelegate, NSSecureCoding {
    ...
    private var _shadowColor1: UIColor?
     ...
     */
     @objc open var shadowColor1: UIColor? {
        set {
            if (_shadowColor1 == newValue) {
                return
            }
            _shadowColor1 = newValue
            innerText.bs_shadow = _shadowFromProperties()
            if innerText.length != 0 && !ignoreCommonProperties {
                if displaysAsynchronously && clearContentsBeforeAsynchronouslyDisplay {
                    _clearContents()
        ...
        get {
            return _shadowColor1
        }
    }
    
    private var _shadowOffset1 = CGSize.zero
    /**
     ...
     */
    @objc open var shadowOffset1: CGSize {
        set {
            if _shadowOffset1 == newValue {
                return
            }
            _shadowOffset1 = newValue
            innerText.bs_shadow = _shadowFromProperties()
            if innerText.length != 0 && !ignoreCommonProperties {
                if displaysAsynchronously && clearContentsBeforeAsynchronouslyDisplay {
                    _clearContents()
    ...
    private func _shadowFromProperties() -> NSShadow? {
        if !(shadowColor1 != nil) || shadowBlurRadius < 0 {
            return nil
        }
        let shadow = NSShadow()
        shadow.shadowColor = shadowColor1
        #if !TARGET_INTERFACE_BUILDER
        shadow.shadowOffset = shadowOffset1
        #else
        shadow.shadowOffset1 = CGSize(width: shadowOffset1.x, height: shadowOffset1.y)
        #endif
    ...
    private func _updateOuterTextProperties() {
        ...
        _shadowColor1 = shadow?.shadowColor as! UIColor?
        // TARGET_INTERFACE_BUILDER
        _shadowOffset1 = shadow?.shadowOffset ?? .zero
