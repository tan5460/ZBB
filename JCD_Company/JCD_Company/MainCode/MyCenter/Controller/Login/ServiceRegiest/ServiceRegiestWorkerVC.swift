...
class ServiceRegiestWorkerVC: BaseViewController, CompanyTypePickerDelegate, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    private let workTypeLabel = UILabel().text("请选择工种类型(可多选)").textColor(.kColor99).font(14)
    private var workTypeTitles: [String] = []
    private let workTypes = AppData.workTypes
    private var workTypeStr = ""
    private var tagTitles = ["做工精细", "专业过硬", "服务极好", "价格合理"]
    ...
    private var isShowWorkerTypes: Bool = false
    ...
    override func viewDidLoad() {
        ...
        workTypes.forEach { (dic) in
            workTypeTitles.append(Utils.getReadString(dir: dic, field: "label"))
        }
        if regiestModel != nil {
            ...
            regiestModel?.workType?.components(separatedBy: ",").forEach({ (work) in
                workTypes.enumerated().forEach { (item) in
                    let index = item.offset
                    let dic = item.element
                    if work == Utils.getReadString(dir: dic, field: "value") {
                        currentWorkTypeIndexs.append(index)
                        if workTypeStr != "" {
                            workTypeStr.append(",")
                        }
                        workTypeStr.append(work)
                    }
                }
            })
            isShowWorkerTypes = true
...
extension ServiceRegiestWorkerVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 3:
            ...
            if !workTypeText.isEmpty {
                ...
                workTypeLabel.text("请选择工种类型(可多选)").textColor(.kColor33)
            }
            if isShowWorkerTypes {
                let arrowImage = #imageLiteral(resourceName: "regiest_arrow_down").rotated(by: .pi)
                arrow.image(arrowImage)
                cell.layout(
                    13.5,
                    |-14-star.width(7).height(20).centerVertically()-0-icon.size(16.64)-8-workTypeLabel.height(44)-0.5-arrow.size(44)-0.5-|,
                    >=0
                )
    ...
    @objc private func arrowBtnClick(btn: UIButton) {
        isShowWorkerTypes = !isShowWorkerTypes
...
// MARK: - 按钮点击方法
extension ServiceRegiestWorkerVC {
    @objc private func workTypeBtnClick(btn: UIButton) {
        ...
        currentWorkTypeIndexs.forEach { (index) in
            ...
            workTypeStr.append(Utils.getReadString(dir: workType, field: "value"))
        }
        tableView.reloadData()
    }
    ...
    @objc private func sureBtnClick(btn: UIButton) {
        ...
        parameters["workType"] = workTypeStr
