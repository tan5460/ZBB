...
//    UIDevice.current.isProximityMonitoringEnabled = true
  ...
  @objc func trackAudio() {
    player.updateMeters()
    if let block = playProgressCallback {
        block(self.identify, player.averagePower(forChannel: 0), player.currentTime, player.duration)
    }
  }
  ...
  @objc open func stopAudio() {
    ...
    if self.player != nil {
        ...
        updater?.invalidate()
        updater = nil
        playProgressCallback = nil
        playFinishCallback = nil
        
        playStopCallback?(self.identify)
        playStopCallback = nil
