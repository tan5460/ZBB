...
open class IMUIMessageCollectionView: UIView {
    ...
    @objc open func updateInsertMessage(with message: IMUIMessageProtocol) {
        
        DispatchQueue.main.async {
            self.chatDataManager.updateMessage(with: message)
            self.reloadDataNoScroll()
        }
    }
...
//  }
    ...
    @objc open func updateMessage(with message: IMUIMessageProtocol, isRefresh: Bool = true) {
        
        DispatchQueue.main.async {
            self.chatDataManager.updateMessage(with: message)
            
            if isRefresh {
                if let index = self.chatDataManager.index(of: message) {
    ...
    @objc open func reloadDataNoScroll() {
        var currentOffset = messageCollectionView.contentOffset
        let contentSizeBeforeInsert = messageCollectionView.collectionViewLayout.collectionViewContentSize
        
        messageCollectionView.reloadData()
        
        let contentSizeAfterInsert = messageCollectionView.collectionViewLayout.collectionViewContentSize
        ...
        messageCollectionView.setContentOffset(currentOffset, animated: false)
    }
...
extension IMUIMessageCollectionView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, didEndDisplaying: UICollectionViewCell, forItemAt: IndexPath) {
        ...
        if let did = didEndDisplaying as? IMUIMessageCellProtocol {
        
            did.didDisAppearCell()
