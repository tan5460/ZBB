...
open class IMUIBaseMessageCell: UICollectionViewCell, IMUIMessageCellProtocol {
    @objc public static var avatarCornerRadius: CGFloat = 45/2
    ...
    var bubbleContentType: MessgeType = .defaulted
    ...
    override init(frame: CGRect) {
        ...
        readLabel.font = IMUIMessageCellLayout.readLabelTextFont
    ...
    func layoutCell(with layout: IMUIMessageCellLayoutProtocol, viewCache: IMUIReuseViewCache) {
        ...
        self.readLabel.frame = layout.readLabelFrame
    ...
    func removeBubbleContentView(viewCache: IMUIReuseViewCache, contentType: MessgeType) {
    ...
    func setupData(with message: IMUIMessageModelProtocol) {
        ...
        if let imageUrl = URL(string: self.urlString!) {
            
            self.avatarImage.kf.setImage(with: imageUrl, placeholder: message.fromUser.Avatar())
        }else {
            self.avatarImage.image = message.fromUser.Avatar()
        }
        ...
        if message.isOutGoing {
            ...
            if message.isRead {
                ...
                self.readLabel.textColor = PublicColor.placeholderTextColor
            }else {
                self.readLabel.text = "未读"
                self.readLabel.textColor = PublicColor.emphasizeColor
            }
