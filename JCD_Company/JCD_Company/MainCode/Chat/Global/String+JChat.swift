...
extension String {
    static func errorAlert(_ error: Error) -> String {
        var errorAlert: String = ""
        
        if error._code > 860000 {
            let  errorcode = JMSGSDKErrorCode(rawValue: Int(error._code))
        ...
        if error._code > 800000 && error._code < 820000  {
            let errorcode = JMSGTcpErrorCode(rawValue: UInt(error._code))
            switch errorcode! {
            ...
            if error._code == 809002 || error._code == 812002 {
                errorAlert = "你已不在该群"
            }
        }
        
        if error._code < 600 && error._code > 0 {
            let errorcode = JMSGHttpErrorCode(rawValue: UInt(error._code))
            switch errorcode! {
        ...
        if error._code == 869999 {
            errorAlert = "网络连接错误"
        }
        if error._code == 898001 {
            errorAlert = "用户名已存在"
        }
        if error._code == 801006 {
            errorAlert = "账号已被禁用"
        }
        if errorAlert == "" {
            errorAlert = "未知错误"
