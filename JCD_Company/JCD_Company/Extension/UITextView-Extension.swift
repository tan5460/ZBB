...
extension UITextView {
    ...
    fileprivate struct RuntimeKey {
        static let placeHolderEx : UnsafeRawPointer! = UnsafeRawPointer.init(bitPattern: "PLACEHOLDEL".hashValue)
        ...
        static let placeHolderExLabel : UnsafeRawPointer! = UnsafeRawPointer.init(bitPattern: "PLACEHOLDELABEL".hashValue)
    ...
    var placeHolderEx: String? {//占位符
        set {
            objc_setAssociatedObject(self, UITextView.RuntimeKey.placeHolderEx, newValue, .OBJC_ASSOCIATION_COPY_NONATOMIC)
            initplaceHolderEx(placeHolderEx!)
        }
        get {
            return  objc_getAssociatedObject(self, UITextView.RuntimeKey.placeHolderEx) as? String
        }
    }
    ...
    var placeHolderExLabel: UILabel? {
        set {
            objc_setAssociatedObject(self, UITextView.RuntimeKey.placeHolderExLabel, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
        get {
            return  objc_getAssociatedObject(self, UITextView.RuntimeKey.placeHolderExLabel) as? UILabel
        }
    }
    ...
    var placeholdFont: UIFont? {
        set {
            ...
            if self.placeHolderExLabel != nil {
                self.placeHolderExLabel?.font = placeholdFont
            }
        }
        get {
    ...
    var placeholdColor: UIColor? {
        set {
            ...
            if self.placeHolderExLabel != nil {
                self.placeHolderExLabel?.textColor = placeholdColor
            }
        }
        get {
     ...
     *  占位符
     */
    fileprivate func initplaceHolderEx(_ placeHolderEx: String) {
        ...
        self.placeHolderExLabel = UILabel()
        placeHolderExLabel?.font = self.placeholdFont
        placeHolderExLabel?.text = placeHolderEx
        placeHolderExLabel?.numberOfLines = 0
        placeHolderExLabel?.lineBreakMode = .byWordWrapping
        placeHolderExLabel?.textColor = self.placeholdColor
        let rect = placeHolderEx.boundingRect(with: CGSize(width: self.frame.size.width-14,
                                                           height: CGFloat(MAXFLOAT)),
                                              options: .usesLineFragmentOrigin,
                                              attributes: [NSAttributedString.Key.font : self.placeholdFont!],
                                              context: nil)
        placeHolderExLabel?.frame = CGRect(x: 7, y: 7, width: rect.size.width, height: rect.size.height)
        addSubview(self.placeHolderExLabel!)
        placeHolderExLabel?.isHidden = self.text.count > 0 ? true : false
    }
    ...
    @objc fileprivate func textChange(_ notification : Notification) {
        ...
        if placeHolderEx != nil {
            placeHolderExLabel?.isHidden = true
            if self.text.count ==  0 {
                self.placeHolderExLabel?.isHidden = false
            }
        }
        if limitLength != nil {
    ...
    override open func layoutSubviews() {
        ...
        if placeHolderEx != nil && placeHolderExLabel != nil {
            let rect: CGRect = placeHolderEx!.boundingRect(with: CGSize(width: frame.width-7, height: CGFloat.greatestFiniteMagnitude), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font: self.placeholdFont!], context: nil)
            placeHolderExLabel!.frame = CGRect(x: 7, y: 7, width: rect.size.width, height: rect.size.height)
        }
        if autoHeight == true {
