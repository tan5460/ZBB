...
# 【配置上传到蒲公英相关信息】(可选)
__PGYER_U_KEY="b639fc96246862a9eabe4b14d56c0fae"
__PGYER_API_KEY="c16752e4aabae22db149b0a35f75a2f4"

# 【配置上传到 Fir】(可选)
__FIR_API_TOKEN="b1103b38b74681bbadc082444622fd70"
...
# 发布APP Store 账号密码
__IOS_SUBMIT_ACCOUNT="986869460@qq.com"
__IOS_SUBMIT_PASSWORD="Chaoyun718**"
...
# 1. 请选择 SCHEME
__SELECT_TARGET_OPTIONS=("1.JCD_Company_Test" "2.JCD_Company" "3.JCD_Company_PRE")
...
if [[ $__SELECT_TARGET_OPTION -eq 1 ]]; then
  __BUILD_TARGET="JCD_Company_Test"
  __SCHEME_NAME="JCD_Company_Test"
elif [[ $__SELECT_TARGET_OPTION -eq 2 ]]; then
  __BUILD_TARGET="JCD_Company"
  __SCHEME_NAME="JCD_Company"
elif [[ $__SELECT_TARGET_OPTION -eq 3 ]]; then
  __BUILD_TARGET="JCD_Company_PRE"
  __SCHEME_NAME="JCD_Company_PRE"
else
...
if [[ $__SELECT_TARGET_OPTION -eq 1 ]]; then
  __CURRENT_INFO_PLIST_NAME="Test-Info.plist"
elif [[ $__SELECT_TARGET_OPTION -eq 2 ]]; then
  __CURRENT_INFO_PLIST_NAME="SupportingFiles/Info.plist"
elif [[ $__SELECT_TARGET_OPTION -eq 3 ]]; then
  __CURRENT_INFO_PLIST_NAME="PRE-Info.plist"
else
    __CURRENT_INFO_PLIST_NAME="PRE-Info.plist"
  exit 1
fi
